...
Deploying massive Vision-Language-Action (VLA) models and dense reconstruction pipelines on energy-constrained robotic platforms presents a profound computational challenge known as the ``memory wall''~\cite{edge_opt2024}. Traditional full-model fine-tuning is computationally prohibitive for edge devices. To mitigate this, Low-Rank Adaptation (LoRA) has emerged as the standard parameter-efficient fine-tuning (PEFT) method. LoRA freezes pre-trained model weights and injects trainable rank decomposition matrices into existing layers, effectively reducing the number of trainable parameters by up to 10,000 times and significantly decreasing VRAM overhead during adaptation~\cite{hu2021lora, wang2024lorafusion}. Furthermore, because the updates are linear, LoRA adapters can be mathematically merged into the base model weights prior to deployment, introducing zero additional inference latency~\cite{11042839}.
...
This survey has systematically reviewed the convergence of 3D~Gaussian Splatting, Vision-Language-Action models, and predictive Next-Best-View planning. By synthesizing these domains, we articulated the Semantic-Metric Loop, a unified architectural framework that tightly couples cognitive semantic reasoning with metric spatial perception. Quantitative evidence consolidated from the literature demonstrates that integrating VLA models---optimised via parameter-efficient techniques such as LoRA and INT8 quantisation---with explicit geometric systems like 3DGS is computationally viable on unified-memory edge hardware (VRAM $\approx 8$--$12$\,GB, latency $< 1$\,s). This integration facilitates a necessary paradigm shift from passive, purely geometric mapping to active, intent-driven autonomous exploration.

Despite these advancements, deploying such closed-loop architectures in safety-critical and unstructured environments introduces substantial challenges. The sim-to-real transfer gap for semantic spatial policies remains a primary bottleneck, requiring robust domain adaptation strategies. Furthermore, the inherent probabilism of VLA outputs demands rigorous safety certification frameworks, such as the integration of Control Barrier Functions, to validate non-deterministic trajectories prior to execution. In active industrial sites, the continuous presence of moving machinery also necessitates the evolution of static 3DGS representations into temporal 4D-GS models to actively filter out dynamic navigation hazards.

Future research must address the scalability and persistence of these systems. Prioritizing decentralized multi-agent coordination under severe bandwidth constraints, alongside energy-aware, long-horizon mission planning, is essential for large-scale infrastructure inspection. Addressing these fundamental challenges represents a critical prerequisite for maturing semantic-guided UAVs from experimental frameworks into reliable, autonomous assets capable of operating safely in highly demanding domains, such as open-pit mining.
% ==============================================================================
% ACKNOWLEDGMENT
% ==============================================================================
