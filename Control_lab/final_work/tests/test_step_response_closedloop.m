...
% Evalúa el sistema CON controlador (Fuzzy + PID + Lag Compensator integrado)
%
% NOTA: El Lag Compensator está ahora integrado en fuzzy_yaw_rate_controller.m
%
...
fprintf('  (Lag Compensator integrado en el controlador)\n\n');
...
while step < max_steps && t_current < max_time
    ...
    % Control Fuzzy + PID (con Lag Compensator integrado internamente)
    params.Kp_wheel = 30;
    params.Ki_wheel = 50;
    params.Kd_wheel = 0;
...
fprintf('\n✓ Simulación completada (%d pasos, %.1f s)\n\n', final_step, t_current);
...
energy = trapz(t_history, sum(U_history.^2, 2));
